...
# ==========================================
try:
    f_reg_raw = st.session_state.get("fecha_registro", datetime.now().isoformat())
    
    # Convertimos la fecha usando Pandas para manejar de forma transparente cualquier zona horaria Z o +00:00
    fecha_reg_dt = pd.to_datetime(f_reg_raw).tz_localize(None)
    fecha_actual_dt = pd.to_datetime(datetime.now()).tz_localize(None)
    
    dias_usados = (fecha_actual_dt - fecha_reg_dt).days
except Exception:
    dias_usados = 0
...
# ==========================================
    ...
    with st.container(border=True):
        ...
        with st.form("form_nueva_meta", clear_on_submit=True):
            ...
            nueva_meta_nombre = col_f1.text_input("¿Para qué estás ahorrando?", placeholder="Ej: Fondo para Plotter / Aguinaldos")
    ...
    st.markdown("---")
    if df_metas.empty:
        st.info("📌 Todavía no tenés ninguna alcancía creada. Podés agregar la primera arriba.")
    else:
        ...
        for idx, row in df_metas.iterrows():
            with st.container(border=True):
                ...
                with col_m3:
                    ...
                    with col_btns[0]:
                        if st.button("💾", key=f"btn_save_m_meta_{row['id']}", help="Guardar nuevo acumulado"):
