...
# --- CONEXIÓN A SUPABASE ---
# Se asume que las credenciales de Supabase ya están configuradas en tus secretos
...
with st.sidebar:
    ...
    # Selección de Rol
    rol_actual = st.selectbox("Rol de Usuario", ["Admin", "Empleado"], index=0 if st.session_state.rol == "Admin" else 1)
    st.session_state.rol = rol_actual
    
    st.markdown("---")
    
    # Secciones dinámicas según el Rol
    if rol_actual == "Admin":
        secciones = [
            "📊 Dashboard General",
            "🤖 Consultor IA",
